...
- ✅ Phase 6 — `a3000-app::tabs::slicer` Remix : pipeline 3 étages (Shuffle → Repeat → Stutter) avec intensités indépendantes + 4 modes shuffle (Random / Beat-aligned / Pair-swap / Block-reorder). PRNG déterministe ChaCha8 seedé par `remix_seed`. Preview audio en boucle (`Playback::start_loop` interleaved f32) + drag-OUT MIDI via nouvelle fonction `a3000_core::midi::generate_midi_sequence` (events absolus note/start_tick/end_tick). Deps ajoutées : `rand` + `rand_chacha` (workspace).
