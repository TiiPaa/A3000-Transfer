...
- ✅ Phase 6 — `a3000-app::tabs::slicer` Remix : pipeline 3 étages (Shuffle → Repeat → Stutter) avec intensités indépendantes + 3 modes shuffle (Random / Pair-swap / Block-reorder), 3 seeds PRNG ChaCha8 indépendants → re-roll par étage via ↻ ou ↻ All. Layout vertical 4 lignes (header + 3 étages alignés). Preview audio en boucle (`Playback::start_loop` interleaved f32) + drag-OUT MIDI via `a3000_core::midi::generate_midi_sequence` (events absolus note/start_tick/end_tick). Deps ajoutées : `rand` + `rand_chacha` (workspace).
- ✅ Phase 7 — Slicer edition : sélection range (Shift+drag, drag edges, Ctrl+click extend, Échap clear) + Crop + Loop sel (playhead suit la sélection) + Undo stack 20 snapshots (Ctrl+Z) + time-stretching Warp via Alt+drag d'onset (Linear resampling ou WSOLA pitch-preserving, nouveau module `time_stretch.rs` 250 LOC pur Rust, 5 tests). Click droit Upload → "Send to Slicer". Auto-restart du Playback après modifs (selection / warp / clear / remix regen).
