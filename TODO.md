...
- [x] Fix transfert depuis lecteur réseau : le worker élevé (UAC, token admin distinct) ne voit pas les lecteurs mappés (`Z:\`) ni les credentials UNC → `NotFound`. Fix : `app::stage_for_worker` copie le WAV en local (`%TEMP%\a3000_staged\<idx>_<nom>`) côté GUI (qui a accès au réseau) si `drive_is_remote` (UNC `\\…` ou `GetDriveTypeW == DRIVE_REMOTE`), passe le chemin temp au worker. Auto-cleanup du temp via `UploadItem.staged_temp` + `cleanup_staged()` sur Done/Error.
- [x] Settings : section "Fichiers temporaires" + bouton "Nettoyer les fichiers temporaires" → vide les 5 sous-dossiers `%TEMP%\a3000_*` (staged/extracted/slices/midi/remix_midi ; `a3000_downloads` exclu). Feedback nb fichiers + MB libérés. `app::cleanup_temp_dirs()`.
